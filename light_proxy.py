...
def requestNodesCpu():
    ...
    if len(client.containers.list()) != 0:
        #the total_cpu_usage means average cpu usage in below.
...
def setMaxAndMin(lower_size,upper_size):
    if request.method == "POST":
        ...
        result = 'Light Pod is now Elastic and the lower size is: '+ lower_size + ' and the upper size is: '+ upper_size
        return jsonify({"result" : result})
@app.route('/cloudproxy/<podId>/nodes/<name>', methods = ['POST'])
def node_register(podId, name):
    if request.method == "POST":
        if len(nodes) >= maxnodes:
            result = 'Pod capacity has been reached (max '+ str(maxnodes) +' nodes)'
            return jsonify({"result" : result})
...
def launch_node(container_name, port_number):
    ...
    [img, logs] = client.images.build (path='./', rm=True ,dockerfile = './Dockerfile' )
    container = client.containers.run(image=img, detach=True, name=container_name, command=['python' , 'test.py', container_name],ports={'5000/tcp' : port_number}, tty=True, cpu_quota = 30000, mem_limit = '100m')
